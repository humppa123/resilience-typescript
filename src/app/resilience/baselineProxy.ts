 ...
 */
export class BaselineProxy implements ResilienceProxy {
  /**
  ...
  constructor(
    ...
    this.maxSampleDuration = new Date(
      now.getTime() + startSamplingAfter + maxSampleDuration
    );
  ...
  async execute<TResult>(
    ...
    if (this.isSamplingFinished) {
      if (this.alarmLevel > 0 && milliseconds >= this.alarmLevel) {
        this.logger.warning(
          requestId,
          `returned above alarm level in ${milliseconds}/${
            this.alarmLevel
          }ms ${this.getStatisticsString()}`,
          null,
          logFormatter
        );
      } else {
      ...
      this.logger.information(
        requestId,
        `returned in ${milliseconds}ms. Sampling ongoing.`,
        null,
        logFormatter
      );
    }
  ...
  private sample(value: number): void {
    ...
    if (
      now > this.maxSampleDuration.getTime() ||
      this.samples.length >= this.maxSamplesCount
    ) {
