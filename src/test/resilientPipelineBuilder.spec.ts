...
describe('Resilence', () => {
  describe('Resilient Pipeline Builder', () => {
    it('Should return func result if all proxies succeed', async () => {
      // Arrange
      const pipeline = ResilientPipelineBuilder.New()
    ...
    it('Should throw error if func throws', async () => {
      ...
      // Assert
      await expect(
        pipeline.execute(func, Guid.createEmpty())
    ...
    it('Should throw error on doubled positions in circuit breaker', done => {
      ...
      expect(() =>
        pipeline.useCircuitBreaker(
          doubledPosition,
          TimeSpansInMilliSeconds.oneHundredMilliSeconds,
          TimeSpansInMilliSeconds.tenMinutes,
          5
        )
      ).to.throw();
    ...
    it('Should throw error on doubled positions in custom proxy', done => {
      ...
      // Assert
      expect(() =>
        pipeline.addProxy(doubledPosition, new RetryProxy(2, new NoLogger()))
      ).to.throw();
