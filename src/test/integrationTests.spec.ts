...
describe('Resilence', () => {
  describe('Integration tests', () => {
    ...
    const tokenProvider: TokenProvider = new AzureActiveDirectoryAppRegistrationTokenProvider(
      baseUrlToken,
      clientId,
      clientSecret,
      tenantId,
    ...
    const baseUrl =
      'https://resilience-typescript.azurewebsites.net/api/persons';
    ...
    it('Should throw an error when could not retrieve a proper AAD token', async () => {
      // Arrange
      const defectTokenProvider: TokenProvider = new AzureActiveDirectoryAppRegistrationTokenProvider(
        baseUrl,
        clientId,
        'This is a wrong secret',
        tenantId,
        logger
      );

      // Act
      const result = async () => defectTokenProvider.getToken();
    ...
    it('CRUD list', async () => {
      // Arrange
      const proxy = ResilientWebPipelineBuilder.New()
        .useCustomLogger(logger)
        .useTokenCache(tokenCache)
        .useCircuitBreaker(
          1,
          TimeSpansInMilliSeconds.tenMinutes,
          TimeSpansInMilliSeconds.tenMinutes,
          10
        )
        .useRetry(2, 30)
